...
async fn test_both_servers_start_and_respond() {
    ...
    let mcp_handle = start_mcp_server(pool.clone(), mcp_port, None, None, None, None, None);
...
async fn test_grpc_server_serves_on_configured_port() {
    ...
    let _handle = start_grpc_server(pool, GrpcServerConfig { port, transport: "tcp".to_string(), socket_path: "/tmp/mongocore.sock".to_string(), socket_permissions: 0o600, max_message_size: 64 * 1024 * 1024, compression: "none".to_string(), stream_idle_timeout_secs: 60, pipeline_timeout_secs: 30, pipeline_max_concurrency: 20 }, None, None, None, None);
...
async fn test_shared_pool_across_servers() {
    ...
    let _mcp_handle = start_mcp_server(pool.clone(), mcp_port, None, None, None, None, None);
    tokio::time::sleep(std::time::Duration::from_millis(150)).await;
