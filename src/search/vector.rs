...
mod tests {
    ...
    fn test_build_pipeline_structure() {
        ...
        let pipeline =
            VectorSearchBuilder::build_pipeline("my_index", "embedding", vector.clone(), 100, 10);
    ...
    fn test_build_pipeline_add_fields_stage() {
        let pipeline =
            VectorSearchBuilder::build_pipeline("idx", "vec_field", vec![1.0, 2.0], 50, 5);
    ...
    fn test_build_pipeline_empty_vector() {
        let pipeline = VectorSearchBuilder::build_pipeline("test_index", "path", vec![], 10, 5);
